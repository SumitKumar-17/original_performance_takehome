...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        # Load parameters from memory (don't hardcode for portability)
        init_slots = []
        for i, v in enumerate(init_vars):
            init_slots.append(("load", ("const", tmp_init, i)))
            init_slots.append(("load", ("load", self.scratch[v], tmp_init)))
        ...
        # Store final results (both indices and values)
        store_slots = []
        ...
        for block in range(blocks_per_round):
            ...
            store_slots.append(
                ("alu", ("+", tmp_addr2, self.scratch["inp_values_p"], offset))
            )
            store_slots.append(
                ("store", ("vstore", tmp_addr, idx_base + block * VLEN))
            )
            store_slots.append(
                ("store", ("vstore", tmp_addr2, val_base + block * VLEN))
            )
